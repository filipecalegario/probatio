...
public class Frequency
{
	...
	public static void main(String[] args) {
		Frequency f = Frequency.ofMidiNote(60);
		System.out.println(f.asHz());
		Frequency f2 = Frequency.ofMidiNote(69);
		System.out.println(f2.asHz());
		//System.out.println(Note.getStringForNote((byte) 60));
		//System.out.println(Note.getFrequencyForNote((byte)60));
		String note = org.jfugue.theory.Note.getToneString((byte) 60);
		Note noteU = new Note("C#");
		System.out.println(noteU.getValue());
		System.out.println(note);
		Intervals intervals = new Intervals("1 2 b3 4 5 #5 b7 8");
		Scale scale = new Scale(intervals);
		Note root = new Note((byte)69);
		Key key = new Key(root, scale);
		Chord chord = new Chord(root, intervals);
		Note[] notes = Scales.getNotesFromRootAndIntervalName((byte)60, "Oriental");//chord.getNotes();
		for (int i = 0; i < notes.length; i++) {
			byte value = notes[i].getValue();
			String name = Note.getToneString(value);
