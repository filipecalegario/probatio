...
public class MapperManagerProbatio implements BlockParserObserver{
	...
	public void onAddBlockEvent(BlockController blockController) {
		for (int i = 0; i < blockController.getDataSize(); i++) {
			...
			blockController.getSignals()[i] = MapperManagerProbatio.addOutput(blockNameById + "-" + blockValueLabel, 1, 'i', "unit", 0, 255);
		}
	}
	...
	public void onRemoveBlockEvent(BlockController blockController) {
		for (int i = 0; i < blockController.getDataSize(); i++) {
			if (blockController.getSignals()[i] != null) {
				try {
					MapperManagerProbatio.removeOutput(blockController.getSignals()[i]);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
