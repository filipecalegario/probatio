...
public class SerialTest_v_0_0_1 extends PApplet{
	...
	public void setup () {
		...
		output = createWriter("report/report_" + new SimpleDateFormat("yyyy_MM_dd_HH_mm_ss_E").format(new Date()) + ".txt");
		output.println("Base: Arduino UNO");
		output.println(BlockType.getBlockStringCodeById(BlockType.RESTOUCH)+": ATTiny 85 1Mhz");
		output.println(BlockType.getBlockStringCodeById(BlockType.CRANK)+": Nano V3.0 Arduino-compatible 16Mhz");
		output.println(BlockType.getBlockStringCodeById(BlockType.BELLOWS)+": ATTiny 85 1Mhz");
		output.println(BlockType.getBlockStringCodeById(BlockType.TURNTABLE)+": ATTiny 85 1Mhz with interruptions");
			...
			if(serialIsReady){
				...
				if(meusInts.length == 14){
					...
					if((millis() - outputPrintTime) < 500){
						output.println(blocksForOutputFile + "" +
								averageS + "#" +	
								maxS + "#" +
								minS + "#" +
								meioS + "#" +
