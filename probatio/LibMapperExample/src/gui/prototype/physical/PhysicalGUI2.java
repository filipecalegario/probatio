...
public class PhysicalGUI2 extends PApplet{
	...
	public void setup() {
		...
		state = State.RUN;
		connections = new HashMap<PhysicalGUI2.BlockIconButton, PhysicalGUI2.BlockIconButton>();
	...
	private void controlP5Setup() {
		cp5 = new ControlP5(this);
		cp5.setAutoDraw(false);
		cp5.enableShortcuts();
		btn = cp5.addButton("colorA")
				.setValue(0)
				.setPosition(0,0)
				.setSize(200,19)
				.setLabel("Current State: " + state);
		Group g3x3 = cp5.addGroup("g3x3");
		BlockIconButton[] mbs = new BlockIconButton[9];
		for (int i = 0; i < mbs.length; i++) {
			mbs[i] = new BlockIconButton(cp5, "b"+i, "bellows");
			mbs[i].setSize(71,71);
			mbs[i].setGroup(g3x3);
		}
	...
	private void addBlockControlP5Setup() {
		cp5 = new ControlP5(this);
		cp5.setAutoDraw(false);
		BlockIconButton[] array = new BlockIconButton[9];
		for (int i = 0; i < array.length; i++) {
			array[i] = new BlockIconButton(cp5, "add"+i, "");
	...
	public void draw() {
		...
		if(state == State.ADDBLOCK){
			...
			//runControlP5.getGroup("g3x3").show();
			cp5.draw();
		}
		//		switch (state) {
		//		case RUN:
	...
	public static void main(String[] args) {
		PApplet.main(PhysicalGUI2.class.getName());
	}
